...
<section class="w-[240px] flex flex-col gap-{gap}">
    <div class="exception flex justify-between items-end">
        <h1>Login</h1>
        <button class="text-[0.75rem] exception text-sub-color" on:click={()=>isModalOpen = true}>Forgot password?</button>
    </div>
...
{#if isModalOpen}
    <Modal close={()=>isModalOpen = false} className="w-[400px] rounded-xl  p-[2rem] pb-[3rem] bg-bg-color flex flex-col gap-6">
...
<style>
    section div:not(.exception), section button:not(.exception), .send, .modalInput{
