<script lang="ts">
    ...
    const moveCaret = () => {
        ...
        if (isLastLetter) {
            ...
            const lastLetterRect = wordsElements[activeWord]?.children[activeLetter - 1]?.getBoundingClientRect();
    ...
    function isWordCorrect(wordIndex): boolean {
        if (activeWord <= wordIndex) return true;
    ...
    function handleInput(e){
        if (e.key === "Backspace"){
            ...
            if (activeLetter > 0){
                activeLetter--;
                wordsElements[activeWord].children[activeLetter].classList.remove("correct");
                wordsElements[activeWord].children[activeLetter].classList.remove("incorrect");
                moveCaret();
            }
            ...
            if (isCorrectLetter(e.key)){
                wordsElements[activeWord]?.children[activeLetter]?.classList.add("correct")
                activeLetter++
                moveCaret();
            }

           else if (!isCorrectLetter(e.key) && e.key !==" "){
                wordsElements[activeWord]?.children[activeLetter]?.classList.add("incorrect")
                activeLetter++
                moveCaret();
            }
        }
...
<main>
    <div class="text-sub-color flex text-3xl gap-4 relative flex-wrap" bind:this={container}>
        <div class="caret" bind:this={caret}></div>
        {#each words as word, index}
            <div class="word {activeWord === index? 'active':''} {isWordCorrect(index)?'':'incorrect'}" bind:this={wordsElements[index]} >
                {#each word as letter}
...
<span class="opacity-0 word letter absolute w-fit text-3xl correct incorrect" bind:this={textElement}>A</span>
<style>
    .letter{
        position: relative;
    }
    ...
    .word.incorrect {
        ...
        text-shadow: 1px 0 0 var(--bg-color),-1px 0 0 var(--bg-color),0 1px 0 var(--bg-color),1px 1px 0 var(--bg-color),-1px 1px 0 var(--bg-color);
    }
